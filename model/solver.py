...
import os
from layers import Summarizer, TransformerDiscriminator
...
class Solver(object):
    ...
    def build(self):
        ...
        if self.config.mode == 'train':
            ...
            self.e_optimizer = optim.Adam(
                self.summarizer.vae.e_transformer.parameters(),
                lr=self.config.lr)
            self.d_optimizer = optim.Adam(
                self.summarizer.vae.d_transformer.parameters(),
                lr=self.config.lr)
            ...
            self.optimizerA_s = optim.Adam(list(self.actor.parameters())
                                           + list(self.summarizer.s_transformer.parameters())
                                           + list(self.linear_compress.parameters()),
                                           lr=self.config.lr)
    ...
    def AC(self, original_features, seq_len, action_fragments):
        #print(original_features,seq_len,action_fragments,sep='\n')
        """Function that makes the actor's actions, in the training steps where the actor and critic components are not trained"""
        # Compute scores using the transformer-based scorer
        scores = self.summarizer.s_transformer(original_features)  # [seq_len, 1]
        print(scores,len(scores),seq_len)
        ...
        for epoch_i in trange(self.config.n_epochs, desc='Epoch', ncols=80):
            ...
            for batch in range(num_batches):
                ...
                if self.config.verbose:
                    tqdm.write('Training Transformer Encoder...')
                ...
                torch.nn.utils.clip_grad_norm_(self.summarizer.vae.e_transformer.parameters(), self.config.clip)
                self.e_optimizer.step()
                
                #---- Train (decoder/generator) ----#
                if self.config.verbose:
                    tqdm.write('Training Transformer Decoder...')
                ...
                torch.nn.utils.clip_grad_norm_(self.summarizer.vae.d_transformer.parameters(), self.config.clip)
                ...
                #---- Train Scorer and actor-critic ----#
                if self.config.verbose:
                    tqdm.write('Training Scorer, actor and critic...')
                ...
                torch.nn.utils.clip_grad_norm_(list(self.actor.parameters()) + list(self.linear_compress.parameters())
                                           + list(self.summarizer.s_transformer.parameters())+list(self.critic.parameters()), self.config.clip)
    ...
    def evaluate(self, epoch_i):
        ...
        for image_features, video_name, action_fragments in tqdm(self.test_loader, desc='Evaluate', ncols=80, leave=False):
            ...
            os.makedirs(score_save_path.parent, exist_ok=True)
